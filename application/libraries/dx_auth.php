...
class dx_auth
{
	...
	*/
	...
	//利用微博id查找用户名和密码，并以此登录
	function weibo_login($id){
		$remember=true;
		$this->ci->load->model('dx_auth/users', 'users');
		$query=$this->ci->users->get_user_by_weibo_id($id);
		// Get user record
				$row = $query->row();
	...
	//微博注册
	function weibo_register($user){
		$this->ci->load->model('dx_auth/users', 'users');
		$weibo_id=$user['id'];
		$password=$this->_g_password();
		$username='weibo'.$weibo_id;
		$data=array('weibo_id'=>$weibo_id,
					'username'=>$username,
					'password'=>crypt($this->_encode($password)));//加密
		$this->ci->users->create_user($data);
		$this->login($username,$password,$remember=TRUE);//登陆
	}
